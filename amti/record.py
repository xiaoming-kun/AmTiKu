...
三条规矩（用户 2026-09-16 定、2026-09-18 改，写死在代码里）：

1. **带图题、带表题一律不录正文**，但**所有**图题/表题都要**登记原卷位置**
   （哪份 PDF、第几页、图上中下），供人工后期处理。
   旧规矩是"只有 8/11/14/18/19 题的带图题才登记"，2026-09-18 用户改成
   **全部登记**——理由是漏登记一道，后期就漏处理一道。
...
# ⚠️ 这里曾经是 `FIG_KEEP = {"8","11","14","18","19"}`：只登记这几个题号的带图题。
# 用户 2026-09-18 改成**全部登记**（漏登记一道，后期就漏处理一道），
# 那个常量随之删掉——**留着不用的常量比删掉更危险**，下一个人会以为它还在生效。
...
def split_figures(qs: list[dict], *, src: str = "") -> tuple[list, list, list]:
    ...
    但**每一道都进第三份**，附上原卷位置供人工后期处理。

    ⚠️ 旧规矩是"只有 8/11/14/18/19 题的带图题才登记"，用户 2026-09-18 改成
    **全部登记**——漏登记一道，后期就漏处理一道。别再把题号白名单加回来。

    ⚠️ **带表题也应该走这条路，但表还没有判据**（v1 里"表"这个概念根本不存在）。
    台账里记着（`设计/录题缺陷台账.md` D12），等有了表的信号再接进来。
    """
    keep, drop, reg = [], [], []
    for q in qs:
        if not q["figure"]:
...
def report(out_dir: Path) -> str:
    ...
    out = ["# 录题报告", "",
           ...
           "> 规矩：**带图题、带表题一律不录**；**每一道图题都登记原卷位置**"
           "（用户 2026-09-18 改成全部登记，不再只登记 8/11/14/18/19）；",
           "> 没有解析的写「解析无」。", "", "## 一、每份卷子", "",
...
"""
    ...
    # 2026-09-18 改规：**全部图题都登记**（旧规矩只管 8/11/14/18/19）
    check("带图题登记了位置（不再看题号）",
          len(reg) == 1 and reg[0]["题号"] == 8 and reg[0]["位置"] == "中 右",
          str(reg))
    _q3 = dict(qs[0], n="3", figure=True)      # 挑一个**不在旧白名单**里的题号
    _k3, _d3, _r3 = split_figures([_q3], src="x.pdf")
    check("旧白名单外的图题也登记（3 号）", len(_r3) == 1 and _r3[0]["题号"] == 3,
          str(_r3))
    check("登记了的图题正文不入库", not _k3 and len(_d3) == 1, "%s %s" % (_k3, _d3))
